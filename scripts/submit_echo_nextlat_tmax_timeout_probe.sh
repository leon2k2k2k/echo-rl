...
# Submit a fast real-TMax ECHO + NextLat probe:
# - uses cached TMax smoke parquets/images
# - keeps the 4-GPU ECHO/FSDP/vLLM shape
# - enables ECHO world-token CE and NextLat completion-token MSE
# - sets terminal-agent timeout to 60s via qwen3_8b_rl_echo_nextlat_fast_timeout_smoke.yaml
...
export RUN_KIND="${RUN_KIND:-echo-nextlat-tmax-timeout-probe}"
export RUN_ID="${RUN_ID:-${RUN_KIND}-$(date +%Y%m%d-%H%M%S)}"
export CONFIG_PATH="${CONFIG_PATH:-echo_configs/qwen3_8b_rl_echo_nextlat_fast_timeout_smoke.yaml}"
...
echo "Submitting fast TMax timeout ECHO + NextLat probe"
